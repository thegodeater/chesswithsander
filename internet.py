...
class ChessComputer:
    ...
    def minimax(chessboard, depth):
        if depth == 0 or chessboard.is_king_dead(Side.White) or chessboard.is_king_dead(Side.Black):
            return (ChessComputer.evaluate_board(chessboard,0), None)
        else: 
            if chessboard.turn == 0:
                bestscore = -float("inf")
                bestmove = None
                for move in chessboard.legal_moves():
                    print(move in chessboard.legal_moves())
                    new_position = chessboard.make_move(move)
                    score=ChessComputer.evaluate_board(new_position,0)
                    move = ChessComputer.minimax(new_position, depth - 1)
                ...
                for move in chessboard.legal_moves():
                    new_position = chessboard.make_move(move)
                    score, move = ChessComputer.minimax(new_position, depth - 1)
